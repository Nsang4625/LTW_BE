...
public class DashboardController {
    ...
    //   Dang phat trien...
    @PutMapping("/Exam2/Fix/{examId}")
    public ResponseEntity<?> editExamById(@PathVariable("examId") Long examId, @RequestBody Exam newExam){
        return examService.editExamById(examId, newExam);
    }

    @PostMapping("/Exam2/Create")
    public ResponseEntity<?> createExam(@RequestBody Exam exam){
        return examService.createExam(exam);
    }

    @DeleteMapping ("/Exam2/Delete/{examId}")
    public ResponseEntity<?> deleteExamById(@PathVariable("examId") Long examId){
        return examService.deleteExamById(examId);
    }

    @GetMapping("/Exam2/Get/{examId}")
